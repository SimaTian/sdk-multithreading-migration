// FIXED: All file system and environment access routed through TaskEnvironment
// - Path.GetFullPath → TaskEnvironment.GetCanonicalForm
// - Directory.GetFiles with relative paths → resolve via TaskEnvironment.GetAbsolutePath first
// - File.ReadAllLines(Path.GetFullPath(...)) → File.ReadAllLines(TaskEnvironment.GetCanonicalForm(...))
// - File.GetLastWriteTime with relative path → resolve via TaskEnvironment.GetCanonicalForm first
// - Environment.MachineName → TaskEnvironment.GetEnvironmentVariable("COMPUTERNAME"/"HOSTNAME")
// - Environment.GetEnvironmentVariable → TaskEnvironment.GetEnvironmentVariable
// - File.WriteAllText/AppendAllText use TaskEnvironment-resolved paths
using System;
...
namespace FixedThreadSafeTasks.ComplexViolations
{
    ...
    public class BuildLogCollector : Microsoft.Build.Utilities.Task, IMultiThreadableTask
    {
        ...
        public override bool Execute()
        {
            try
            {
                // FIX: Use TaskEnvironment.GetCanonicalForm instead of Path.GetFullPath
                string reportPath = TaskEnvironment.GetCanonicalForm(OutputReportPath);
                ...
                {
                    // FIX: Resolve relative directory via TaskEnvironment.GetAbsolutePath
        ...
        private List<LogEntry> ParseLogFile(string path)
        {
            ...
            // FIX: Use TaskEnvironment.GetCanonicalForm instead of Path.GetFullPath
            string[] lines = File.ReadAllLines(TaskEnvironment.GetCanonicalForm(path));
        ...
        {
            // FIX: Resolve path via TaskEnvironment before accessing file metadata
            string resolvedPath = TaskEnvironment.GetCanonicalForm(filePath);
            DateTime lastWrite = File.GetLastWriteTime(resolvedPath);
            return DateTime.Now - lastWrite;
        }

        private void WriteReportHeader(string reportPath)
        {
            // FIX: Use TaskEnvironment.GetEnvironmentVariable instead of Environment.MachineName
            string machineName = TaskEnvironment.GetEnvironmentVariable("COMPUTERNAME")
                ?? TaskEnvironment.GetEnvironmentVariable("HOSTNAME")
                ?? "unknown";
            // FIX: Use TaskEnvironment.GetEnvironmentVariable instead of Environment.GetEnvironmentVariable
            string buildNumber = TaskEnvironment.GetEnvironmentVariable("BUILD_NUMBER") ?? "local";
