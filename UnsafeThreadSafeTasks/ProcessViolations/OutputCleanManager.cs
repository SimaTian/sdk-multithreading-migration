// VIOLATION: Uses Process.Start() directly to check file locks and force-delete files
// instead of using TaskEnvironment.GetProcessStartInfo() for process creation.
using System;
...
namespace UnsafeThreadSafeTasks.ProcessViolations
{
    ...
    public class OutputCleanManager : Microsoft.Build.Utilities.Task, IMultiThreadableTask
    {
        ...
        public override bool Execute()
        {
            try
            {
                ...
                foreach (ITaskItem dirItem in OutputDirectories)
                {
                    ...
                    // VIOLATION: Uses Path.GetFullPath() instead of TaskEnvironment.GetCanonicalForm()
        ...
        private void CleanDirectory(string dir, string[] preservePatterns)
        {
            ...
            // VIOLATION: Direct filesystem enumeration via Directory.EnumerateFiles
        ...
        private bool TryDeleteFile(string filePath)
        {
            try
            {
                // Check for readonly attribute before attempting delete
        ...
        private bool CheckFileLock(string filePath)
        {
            try
            {
                // VIOLATION: Creates ProcessStartInfo directly instead of using
                ...
                // handle.exe returns lines with process info if the file is locked
        ...
        private static bool ShouldPreserve(string fileName, string[] patterns)
        {
            foreach (string pattern in patterns)
            {
                ...
                // Simple wildcard matching: support leading "*" patterns like "*.license"
                if (trimmed.StartsWith("*.", StringComparison.Ordinal))
                {
                    string extension = trimmed.Substring(1); // e.g. ".license"
